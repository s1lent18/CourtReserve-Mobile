...
fun VendorLanding(
    ...
    Surface {
        ...
        val loginResult = vendorAuthViewModel.loginResult.collectAsState()
                        ...
                        Button(
                            ...
                            onClick = {
                                if (email.isNotEmpty() && password.isNotEmpty()) {
                                    ...
                                    val vendorLoginRequest = VendorLoginRequest(email = email, password = password)
                                    vendorAuthViewModel.vendorLogin(vendorLoginRequest)
                                }
                    ...
                    if (email.isNotEmpty() && password.isNotEmpty() && requestreceived) {
                        when (val result = loginResult.value) {
                            ...
                            is NetworkResponse.Success -> {
                                isLoading = false
                                LaunchedEffect(Unit) {
                                    vendorTokenViewModel.saveVendorData(
                                        vendorData = result.data.vendorData,
